...
**My4S** – a configurator program for building web applications (ERP, CRM, etc).
...
In most cases, you will use the program as a ready‑made product (after configuring it in the configurator for your tasks).  
However, the logic — mathematics and other calculations — you will write yourself in custom modules and functions (or maybe AI will help you ������).
...
In this regard, I decided to make only one table for each object - documents, catalogs, information registers, and so on.
...
This will be possible only for experienced system administrators or programmers.
                                  ...
                                  Database Truncation Mechanism as of a Specific Date
If a database has been in operation for years and has grown significantly, a need arises to reduce its size. In My4s, this is fairly easy to do.
The balance tracking mechanism is organized in such a way that deleting documents up to a month-aligned (discrete monthly) date does not affect final balances.
Note: There is no built-in document deletion mechanism in the My4s user interface. A document can only be set to active (actual) or inactive, which affects register postings during its write operation.
However, you can delete documents directly in the Firebird DBMS:
•	Balances generated by documents will remain unchanged because they are stored in a separate table named BALANSE.
•	You can also delete related documents in the BALANSDOC table up to the same date.
