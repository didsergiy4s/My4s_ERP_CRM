...
then simply copy the My4S folder from the new release to the main My4S folder. If you've already changed something, then check whether your changes are included in the new release's files. Otherwise, you'll overwrite your work!
...
We do this: the server, regardless of the time zone, is always linked to London's zero time zone (winter only!). And for each client, we always calculate the offset relative to London (timezoneOffset) and adjust the client's date by this offset when writing dates to the server.
...
There are often complex report layouts (for example, layouts such as Tax Invoice and others). 
